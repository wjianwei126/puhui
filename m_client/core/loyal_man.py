...
class server_conn(object):
    def __init__(self,config_parms):
        ...
        self.task_dic = {}
        self.last_task_id_file = '%s/%s' %(global_setting.base_dir, self.config_parms['last_task_id'])
    ...
    def get_new_tasks(self):
        ...
        if self.get_last_run_task_id() is True:
            ...
            print '\033[32;1mGetting new task from====>\033[0m',url
    ...
    def get_last_run_task_id(self):
        ...
        #last_task_id_file = '%s/%s' %(global_setting.base_dir, self.config_parms['last_task_id'])
        f = file(self.last_task_id_file, 'rb')
        last_id_raw = f.read()
    ...
    def update_last_task_id(self,new_id):
        #last_task_id_file = '%s/%s' %(global_setting.base_dir, self.config_parms['last_task_id'])
        f = file(self.last_task_id_file, 'wb')
        f.write(str(new_id))
        f.close()
    ...
    def handle(self):
        ...
        while True:
            ...
            if first_run_flag == 1 or reach_interval_flag ==1:
                ...
                print '\033[32;1mhost profile-->\033[0m',self.host_profile
                    ...
                    if self.parse_tasks(has_new_task) is True:
                        ...
                        for task_id,task in self.task_dic.items():
                            if task['kick_off_at'] is not None:
                                ...
                                print 'task_run_time: %s  local_time:%s' %(unix_time,time.time())
                                if time.time() > unix_time: #local time is has reached task run time 
                            ...
                            else: #need to run the task immediately 
                                task_kick_off_flag = True
                            if task_kick_off_flag is True:
                                print 'Going to run this task....'
                                t = threading.Thread(target=self.run, args=[task,])
                                t.start()  
                    #print task
    ...
    def cmd(self,task):
        ...
        return res
    def file_transfer(self,task):
        print '---going to download file----'
        
        return [0,'downloaded file']
    def parse_tasks(self, new_tasks):
        print '\033[34;1mReceived new task:\033[0m---------parsing new tasks--------' #,new_tasks
        ...
        print '\033[34;1mCurrent task pool:\033[0m',self.task_dic
        return True
